...
| `rivr_ota_core.c` | Ondertekende `PKT_PROG_PUSH`-poort: Ed25519-verificatie, anti-replay volgnummer in NVS, `rivr_ota_activate()` / `rivr_ota_confirm()` / `rivr_ota_is_pending()`. Platform-onafhankelijk; koppelt aan `rivr_ota_platform.c` (ESP-IDF NVS) of de nRF52/RP2040-varianten |
| `crypto/hmac_sha256.c` | Zelfstandige FIPS 180-4 SHA-256 + RFC 2104 HMAC-SHA-256; geen heap; stack ≈60 B; getest tegen RFC 4231 TC1 |
