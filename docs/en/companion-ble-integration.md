...
When the companion app sends `RIVR_CP_CMD_APP_START` (0x01) after connecting,
...
### 14.1 Commands (phone → node, via RX characteristic write)
...
### 14.2 Packets (node → phone, via TX characteristic notify)
...
──────  ───  ──────────  ───────  ────────────────────────────────────────
  ...
  4      1   rssi        i8       dBm
  5      1   snr         i8       dB
  6      1   hop_count   u8       1 = direct neighbour
  7      1   link_score  u8       0–100 composite quality
  8      1   role        u8       rivr_node_role_t (0=unknown,1=client,2=repeater,3=gateway)
  9      1   (pad)       u8       0x00
 10     12   callsign    bytes    ASCII, NUL-padded, max 11 chars
 22      4   lat_e7      i32 LE   degrees × 1e7; INT32_MIN = unknown
 26      4   lon_e7      i32 LE   degrees × 1e7; INT32_MIN = unknown
```
...
──────  ───  ──────────  ───────  ────────────────────────────────────────
 ...
 10      1   unit_code   u8       UNIT_* constant (see services.md §4.1)
 11      4   timestamp   u32 LE   Seconds since node boot (0 = unset)
```

See [services.md](services.md#42-built-in-sensor-ids) for built-in sensor IDs (DS18B20, AM2302, VBAT).
