/*
 * variants/heltec_lora32_v4/config.h
 *
 * Build-variant header for:
 *   Board  : Heltec WiFi LoRa 32 V4  (ESP32-S3R8, 8 MB flash, 8 MB OPI PSRAM)
 ...
 *   -include ${PROJECT_DIR}/variants/heltec_lora32_v4/config.h
 *
 * ── Pin mapping  (Heltec WiFi LoRa 32 V4) ────────────────────────────────
 *
 ...
 *  OLED SDA            I²C SDA         17
 *  OLED SCL            I²C SCL         18
 *  VEXT_EN             Peripheral pwr  36   (active-low)
 *
 * ── V4 vs V3 hardware notes ──────────────────────────────────────────────
 *  The V4 uses ESP32-S3R8 (8 MB integrated OPI PSRAM) vs the V3's plain
 *  ESP32-S3.  All LoRa and display GPIO assignments are identical.
 *  PSRAM is not enabled in this firmware build (not required for the mesh
 *  stack); enable CONFIG_SPIRAM in the sdkconfig if heap expansion is
...
#ifndef RIVR_VARIANT_HELTEC_LORA32_V4_H
#define RIVR_VARIANT_HELTEC_LORA32_V4_H
