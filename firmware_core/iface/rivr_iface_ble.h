 ...
 *
 * Wraps NimBLE GATT notify so the bus can push frames to a connected
 * BLE client without knowing NimBLE connection-handle mechanics.
 *
 ...
 * @return true if NimBLE has an active connection
 */
 ...
 * @return true  if the notify was submitted to NimBLE successfully
 * @return false if not connected or NimBLE returned an error
 */
