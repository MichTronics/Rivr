/**
 * @file  firmware_core/rivr_ota_platform_nrf52.c
 * @brief nRF52840 stub for the OTA platform persistence interface.
 *
 * The ESP32 implementation (rivr_ota_platform.c) uses ESP-IDF NVS.
 * The nRF52840 has no NVS; this stub makes all OTA persistence operations
 ...
 * or InternalStorage-based persistence.
 */
...
/* ── In-RAM state (lost on reset — no persistence on nRF52 yet) ─────────── */
...
{
    /* On nRF52 we can't persist to flash yet; log a notice and return success
     * so the OTA core marks the push as accepted (in-RAM only). */
    printf("OTA: program stored in RAM (%u bytes) — not flash-persistent on nRF52\r\n",
           (unsigned)s_prog_len);
    return true;
}
